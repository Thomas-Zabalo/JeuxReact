...
export function mainGame (k) {
    ...
    const spawnMotoBug = () => {
        ...
        const motobug = makeMotobug(k, position);
    ...
    const spawnPoisson = () => {
        ...
        const fish = makeFish(k, position);
        fishs.push(fish);  
    ...
    const spawnPlatformsaut = () => {
        ...
        const platformdesaut = makePlatformSaut(k, position);
    ...
    const spawnExe = () => {
        ...
        const exe = makeSonicexe(k, position);
        exes.push(exe);  
    ...
    // Fin du jeu : Appuyez sur une touche pour afficher l'écran de fin
    k.onKeyPress('end', () => {
        k.go('gameover', { time: elapsedTime }); // Aller à la scène "gameover" avec le temps écoulé
    });
